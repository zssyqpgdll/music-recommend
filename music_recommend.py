...
def get_rid2name(file_name):
    ...
    :return: 两个字典
    id_to_name:键为id，值为name，只有一个键值对
    name_to_id = {}键为name，值为id，只有一个键值对
    """
    id_to_name = {}
    name_to_id = {}
...
item_id_to_name, item_name_to_id = get_rid2name('dataset/new_song_info.txt')
# 得到用户名称到id 和 用户id到名称的映射
user_id_to_name, user_name_to_id = get_rid2name('./dataset/user_in_record.txt')
...
def get_trainset_algo():
    ...
    # 这是一个用户数量N，矩阵大小为 N*N 的稀疏矩阵，然后get_neighbors得到的是topK个相似用户。如果想要得到相似歌曲，则需要使用基于项目的协同过滤算法，或者从得到的相似用户中，提取他们的播放记录（这是基于用户的协同过滤算法）
    ...
    # 选择算法
    # 基于邻域的协同过滤算法：knns.KNNBasic; knns.KNNWithMeans; knns.KNNWithZScore; knns.KNNBaseline;
...
def get_topN_items(current_item_raw_id, topK):
    ...
    # 从相似歌曲的原始id得到相似歌曲的名称
    current_song_neighbors = (item_id_to_name[id]
                              for id in current_song_neighbors)
    # 存储topN首歌曲的id
    topN_items_id = []
    print("推荐歌曲如下：")
    for song in current_song_neighbors:
        topN_items_id.append(item_name_to_id[song])
        print(item_name_to_id[song], song)
...
def get_topN_users(current_user_raw_id, topK):
    ...
    # 从相似音乐好友的原始id得到名字
    current_user_neighbors = (user_id_to_name[rid]
    ...
    for user in current_user_neighbors:
        topN_users_id.append(user_name_to_id[user])
        print(user_name_to_id[user], user)

    return topN_users_id

# 返回前十个最相似的用户
# users_id_list = get_topN_users('507921560', 10)
# init_id = '507921560' + '\t'
...
def get_all_topN_songs():
    ...
    with open(song_info_file_name, 'r', encoding='utf-8') as input_f, open(all_topN_songs_file_name, 'a',
        ...
        for line in input_f:
            ...
            print('获得' + item_id_to_name[song_id] + '的推荐结果：')
            line = song_id + '\t'
            # 将得到的结果拼接成字符串
...
def get_all_topN_users():
    ...
    """
    # 输入文件路径和结果文件路径
    user_info_file_name = './dataset/user_in_record.txt'
